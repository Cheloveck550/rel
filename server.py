...
# ХОСТ в vless:// ДОЛЖЕН быть адресом твоего сервера, а не SNI!
DOMAIN_OR_IP = "64.188.64.214"

# Поскольку derive pbk у тебя не сработал — используем ручной public key от твоего privateKey:
...
def read_vless_from_config() -> Tuple[str, int, str, str, str]:
    ...
    if not private_key:
        raise RuntimeError("privateKey missing")

    return uuid, port, sni, short_id, private_key

def make_vless_link(uuid: str, host: str, port: int, sni: str, pbk: str, short_id: str, use_flow: bool) -> str:
    base = (
...
async def subs_page(token: str):
    ...
    uuid, port, sni, short_id, _priv = read_vless_from_config()
    pbk = PUBLIC_KEY_OVERRIDE
    if not pbk:
        raise HTTPException(status_code=500, detail="PUBLIC_KEY_OVERRIDE пуст — укажи pbk")
...
async def subscription_plain(token: str, noflow: int = Query(0, description="1 — без flow")):
    if not db_has_token(token):
        raise HTTPException(status_code=404, detail="Подписка не найдена")
